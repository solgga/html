...
// div#output 인 요소를 찾아보면
const output = document.querySelector('div#output');

// 배열 선언 & 초기화:
const arr = [1, 2, 30, 40, -5]; // 자바의 경우: int[] arr = {1, 2, 3};

// 배열 arr 의 내용을 output 영역에 써보면
// 첫번째 for 구문
let html = '';
...
// 자바 향상된 for 문장: for (변수 선언 : 배열 이름) { ... }

// 두번째 for 구문 for-of 문장: 배열의 원소들을 iteration(순회)
html=''; // html 초기화
for (let val of arr) {
    html+=`${val} `;
}
...
// 배열 arr 의 원소들의 합계와 평균을 output에 출력해보세요
html='';
