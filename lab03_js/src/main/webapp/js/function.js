...
let result= add(1, 2, 3); // 함수를 호출한 부분
...
result = add(1); // 선언된파라미터보다 적은 개수의 아규먼트를 전달한 경우.
console.log(`result = ${result}`);

// undefined: 초기화되지 않은 변수(값이 할당되지 않은 변수)
// NaN(Not a Number)

// JS 모든 함수는 arguements 속성(property)을 가지고 있음.
function testArgs(){
...
result = multiply (2);    // 아규먼트 y를 전달하지 않으면 기본값ㅇ ㅣ사용됨.
console.log(`result = ${result}`);
...
console.log(plus(100,200)); // plus(100,200): 함수호출
...
//-> 이벤트 리스너(핸들러) 설정할 때 많이 사용되는 코드 패턴들.
