...
<body>
    ...
    <main>
        ...
        <%-- 
        scriptlet: JSP 안에서 자바 코드(_jspService 메서드 안에서 실행되는 코드)들을 작성하기 위한 태그. 
        지역 변수 선언 & 초기화, 객체 생성, 메서드 호출, 조건문, 반복문, ... 
        --%>
        
        <%
        // HTML 테이블을 작성할 때 필요한 더미 데이터
        ...
        <dl>
        <%-- <dt>이름</dt><dd>전화번호</dd><dd>이메일</dd> 반복문으로 --%>
        <% for (Contact c : data) { %>
            <dt><%= c.getName() %></dt><dd><%= c.getPhone() %></dd><dd><%= c.getEmail() %></dd>
        <% } %>
        </dl>
    </main>
</body>
